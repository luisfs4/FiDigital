...
class Direccion extends BaseController
{
	...
	private function sendAjaxResponse($data_filtros, $method)
	{
		...
		$response = $this->DireccionModel->$method($data_filtros);
	...
	public function listado()
	{
		// Title de la pagina
		$data_view = [
			"ruta" => 'Listado de direcciones',
			'scripts' => [
				['src' => base_url('public/js/panel/direcciones/listado.js')]
			]
		];

		// Imprimir vista
		$this->renderView(['panel/direcciones/listado'], $data_view);
	}
	...
	public function post_direccion()
	{
		return $this->sendAjaxResponse($this->request->getPost(), 'post_direccion');
	}
