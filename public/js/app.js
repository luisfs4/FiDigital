...
const cargar_opciones_puntos = async (selector_destino, url, parametros) => {
    ...
    try {
        ...
        if (respuesta && respuesta.length > 0) {
            ...
            respuesta.forEach(({ id_punto, jerarquia, siguiente_disponible, nombre_punto, monto_restante, direccion, programa }) => {
                $(selector_destino).append(`<option value="${id_punto}" monto_restante="${monto_restante}" jerarquia="${jerarquia}" data-direccion="${direccion}" data-programa="${programa}" siguiente_disponible="${siguiente_disponible}">${jerarquia ? jerarquia + ' - ' : ''}${nombre_punto}</option>`);
            });
        } else {
