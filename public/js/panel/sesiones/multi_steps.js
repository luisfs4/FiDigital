...
const countInputsByPanel = (activePanel, activePanelNum, next_btn = 0) => {
	...
	let contador_total = $(activePanel).find('.input_visita').length;

	$(activePanel).find('.input_visita').each(function () {
		if (next_btn == 1) {
...
DOMstrings.stepsBar.addEventListener('click', e => {
	...
	//Guardar la visita al cambiar de pestaña
	if($('.input_visita[name="id_visita"]').val()){
		//guardar_visita();
	}
